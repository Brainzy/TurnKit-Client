...
namespace TurnKit.Internal.ParrelSync
{
    ...
    public class ParrelSyncProjectSettings : ScriptableObject
    {
        ...
        private static string GetScriptableObjectsDirectory()
        {
            var scriptInstance = CreateInstance<ParrelSyncProjectSettings>();
            var script = MonoScript.FromScriptableObject(scriptInstance);
            DestroyImmediate(scriptInstance);
            var scriptPath = AssetDatabase.GetAssetPath(script);
            var editorDirectory = Path.GetDirectoryName(scriptPath)?.Replace('\\', '/');
            var parrelSyncRoot = Path.GetDirectoryName(editorDirectory)?.Replace('\\', '/');
            return $"{parrelSyncRoot}/ScriptableObjects";
        }

        private static string GetSettingsPath()
        {
            return $"{GetScriptableObjectsDirectory()}/{nameof(ParrelSyncProjectSettings)}.asset";
        }

        private static ParrelSyncProjectSettings GetOrCreateSettings()
        {
            var settingsPath = GetSettingsPath();
            var scriptableObjectsDirectory = GetScriptableObjectsDirectory();
            ParrelSyncProjectSettings projectSettings;
            if (File.Exists(settingsPath))
            {
                projectSettings = AssetDatabase.LoadAssetAtPath<ParrelSyncProjectSettings>(settingsPath);

                if (projectSettings == null)
                    Debug.LogError("File Exists, but failed to load: " + settingsPath);
            ...
            if (!Directory.Exists(scriptableObjectsDirectory))
            {
                Directory.CreateDirectory(scriptableObjectsDirectory);
            }
            AssetDatabase.CreateAsset(projectSettings, settingsPath);
